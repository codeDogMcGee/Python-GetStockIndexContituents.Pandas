...
class MasterFile:
    ...
    def index_map_and_all_symbol_list(self):
        ...
        for index in self.index_list:
            df = read_csv(path.join(self.working_directory, f'{index}.csv'))
    ...
    def make_master_dataframe(self):
        ...
        date_string = datetime.strftime(datetime.now(), "%m/%d/%Y %H:%M:%S")
        last_update_list = [date_string] + [''] * (n_rows - 1)

        master_df = DataFrame({'All_Count': all_symbols_count,
